...
/** Machine 制造商 & 型号池 */
const machineManufacturers = ['LUGONG', 'ZOOMLION', 'EP']
const wheelLoaderModels = ['LM930', 'LM920', 'LM938', 'LM946']
const forkliftModels = ['EP-100', 'EP-200', 'EP-300']
...
/** 生成 100 条 Mock 数据 */
export const data: InventoryRow[] = Array.from(
  { length: 100 },
  (_, i): InventoryRow => {
    const id = i + 1
    ...
    if (category === 'Machine') {
      // 在四种机型中随机选
      const kinds = ['Wheel Loader', 'Skid Steer Loader', 'FORKLIFT', 'EXCAVATOR']
      const kind = kinds[Math.floor(Math.random() * kinds.length)]

      if (kind === 'Wheel Loader') {
        manufacturer = 'LUGONG'
        model = wheelLoaderModels[
          Math.floor(Math.random() * wheelLoaderModels.length)
        ]
        ...
        manufacturer = 'EP'
        model =
          forkliftModels[Math.floor(Math.random() * forkliftModels.length)]
      } else {
        // EXCAVATOR
        manufacturer = 'ZOOMLION'
        model = 'ZOOMLION'
