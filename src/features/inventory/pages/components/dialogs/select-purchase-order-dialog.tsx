...
interface SelectPurchaseOrderDialogProps {
  open: boolean
  onClose: () => void
  onSelect: (purchaseOrders: PurchaseOrder[]) => void
  purchaseOrders: PurchaseOrder[]
  multiSelect?: boolean
  isLoading?: boolean
}

type SortField = "orderNumber" | "supplier" | "orderDate" | "expectedDeliveryDate" | "status"
type SortDirection = "asc" | "desc"
type StatusFilter = "all" | "pending" | "partial" | "complete" | "cancelled"

export function SelectPurchaseOrderDialog({
  open,
  onClose,
  onSelect,
  purchaseOrders,
  multiSelect = true,
  isLoading = false,
}: SelectPurchaseOrderDialogProps) {
  ...
  }, [purchaseOrders, searchQuery, statusFilter, dateRange])
  ...
  const handleConfirmSelection = () => {
    const selected = purchaseOrders.filter((order) => selectedOrders[order.id])
    onSelect(selected)
  }
  ...
  const selectedOrdersDetails = useMemo(() => {
    ...
    selected.forEach((order) => {
      order.items.forEach((item) => {
        totalItems += item.orderedQty - item.receivedQty
        totalValue += (item.orderedQty - item.receivedQty) * item.unitPrice
      })
    })
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle>Select Purchase {multiSelect ? "Orders" : "Order"}</DialogTitle>
          <DialogDescription>
            {multiSelect
              ? "Choose one or more purchase orders to receive items from"
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium">
                    {selectedCount === 1 ? "purchase order" : "purchase orders"} selected
                  </span>
                </div>
                <div className="flex items-center space-x-4 text-sm">
                  <div>
                    <span className="font-medium">{selectedOrdersDetails.totalItems}</span> items to receive
                  </div>
                  <div>
