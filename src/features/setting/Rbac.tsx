...
import type { User as UserType, PermissionModule } from "@/lib/types/user-permission"
import {
  getUsers,
...
export default function UserPermissionsPage() {
    ...
    const loadUserPermissions = async () => {
      ...
      try {
        ...
        toast({
          ...
          description: "Unable to load user permissions.",
          variant: "destructive",
        })
      }
    }
  ...
  const savePermissions = async () => {
    ...
    setSaving(true)
    try {
      await updateUserPermissions(selectedUser.id, userPermissions, "Current Admin")
      setOriginalPermissions(userPermissions)
      toast({
        title: "Permissions Saved",
        description: `Successfully updated permissions for ${selectedUser.fullName}.`,
        variant: "success",
      })
    } catch (error) {
      toast({
        title: "Save Failed",
        description: "Failed to save permission configuration. Please try again.",
        variant: "destructive",
      })
    } finally {
      setSaving(false)
  ...
  const getPermissionStats = () => {
    const totalPermissions = permissionModules.reduce((sum, module) => sum + module.permissions.length, 0)
